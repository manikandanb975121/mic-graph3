...
class RequestManager:
    ...
    def _get_events(self, misp_event_filters):
        ...
        events_for_each_filter = [
            ...
            for event_filter in misp_event_filters
        ]
    ...
    def comparison(self):
        compare = self.read_tiindicators2()
        #print("Graph API")
        #print(type(compare))
        #self.graph_list.append(compare)
        id_list = list()
        
        for i in range(0,len(compare['value'])):
            ids = compare['value'][i]['id']
            id_list.append(ids)
        print("Ids from tiindicatros:")
        for i in range(0,len(id_list)):
            print(id_list[i]+"\n")
        #return id_list
        self.list_ids_ = id_list
        print(self.list_ids_)
    
    def comparison_two(self,response):
        #print("response type: "+ str(type(response)))
        #print(len(response['value']))
        res_id = list()
        for i in range(0,len(response['value'])):
            res_id.append(response['value'][i]['id'])
        #print(response['value'][0]['id'])
        list1 = list()
        #a = list(str(response)[88::].split(','))
        #sample = list()
        '''for i in range(0,len(a)):
            if i == 1:
                res_id.append(a[i])'''
                
        '''for i in range(0,len(res_id)):
            a = str(res_id[i])[8:72]
            sample.append(a)'''
        ...
        id_list = self.list_ids_
        for i in range(0,len(id_list)):
            for j in range(0,len(res_id)):
                if id_list[i] == res_id[j]:
                    list1.append(id_list[i])
        ...
        json.dump(list1, open(f'data/{COMMON_DIRECTORY_NAME}/{log_file_name}', 'w'), indent=2)


        for i in range(0,len(list1)):
            request_body = {'value': list1[i]}
            common_events = requests.get(GET_URL+str(list1[i]), headers=self.headers, json=request_body).json()
            #print(type(common_events))
            self.graph_list.append(common_events)
            log_file_name = f"{self._get_datetime_now()}.json"
            json.dump(common_events, open(f'data/{COMMON_EVENTS_DIRECTORY_NAME}/{log_file_name}', 'w'), indent=2)
            external_id = common_events['externalId']
            misp_event_filters = [
                {
                    "uuid": external_id
                }
            ]
            events = self._get_events(misp_event_filters)
            log_file_name = f"{self._get_datetime_now()}.json"
            json.dump(events, open(f'data/{MISP_DIRECTORY_NAME}/{log_file_name}', 'w'), indent=2)
            self.graph_list.append(events[0])
    
           
            log_file_name = f"{self._get_datetime_now()}.json"
            json.dump(self.graph_list, open(f'{MISP_GRAPH}/{log_file_name}', 'w'), indent=2)        
            self.graph_list = []    
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self._print_summary()
        result = self.read_tiindicators2()
        log_file_name = f"{self._get_datetime_now()}_read_Tiindicators.json"
        #json.dump(result, open(f'{READ_DIRECTORY_NAME}/{log_file_name}', 'w'), indent=4)
    ...
    def _find_indicators(self):
        indicators = list(self.hash_of_indicators_to_delete.values())
        self.del_count = len(indicators)
        for i in range(0, len(indicators), 100):
            request_body = {'value': indicators[i: i+100]}
            response = requests.get(GRAPH_TI_INDICATORS_URL, headers=self.headers, json=request_body).json()
            log_file_name = f"del_{self._get_datetime_now()}.json"
            #print(log_file_name)
            #print(json.dumps(response, indent=2))
            #print()
    ...
    def _post_to_graph(self):
        ...
        self.indicators_to_be_sent = []
        self.comparison_two(dict(response))
        self._log_post(response)
